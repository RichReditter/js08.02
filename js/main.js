...
xhr.onload = function() {
    ...
    newObject.map( element =>{
        element.locationNumber = String(element.locationNumber)
        element.locationNumber = element.locationNumber.split('')
        element.locationNumber.forEach( el => {
            if(el === '1'){
                delete newObject[element]                              // не получается удалить элемент из массива
            }
        })
        return newObject
    })
...
xhr.onload = function() {
    ...
    // secondObject = secondObject.map( element => {
    //     splittedBody = element.body.split(' ')
